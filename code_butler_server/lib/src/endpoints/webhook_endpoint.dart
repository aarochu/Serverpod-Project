...
import 'package:code_butler_server/src/services/job_processor.dart';
...
class WebhookEndpoint extends Endpoint {
  ...
  Future<void> handlePullRequest(
    ...
    try {
      ...
      if (signature != null) {
        final secret = session.serverpod.config.extra?['webhookSecret'] as String?;
        if (secret != null && !_verifySignature(payload, signature, secret)) {
          session.log('Invalid webhook signature', level: LogLevel.warning);
