...
class GitPatchService {
  ...
  GitHub? get _github => (githubService as dynamic)._github;
  ...
  Future<void> commitChanges(
    ...
    try {
      ...
      final treeItems = <CreateGitTreeEntry>[];
      for (final entry in filePatches.entries) {
        final blob = await github.git.createBlob(slug, BlobInput(
          content: entry.value,
          encoding: 'utf-8',
        ));
        
        treeItems.add(CreateGitTreeEntry(
          path: entry.key,
          mode: '100644',
          type: 'blob',
          sha: blob.sha,
      ...
      // Create tree
      final tree = await github.git.createTree(slug, CreateGitTreeInput(
        baseTree: baseTree,
        tree: treeItems,
      ));

      // Create commit
      await github.git.createCommit(slug, CreateGitCommitInput(
