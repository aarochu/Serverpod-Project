...
class MetricsEndpoint extends Endpoint {
  ...
  Future<Map<String, dynamic>> getRepositoryHealth(
    ...
    try {
      ...
      final prIds = pullRequests.map((pr) => pr.id!).toSet();
      if (prIds.isEmpty) {
        return {
  ...
  Future<List<Map<String, dynamic>>> getMostProblematicFiles(
    ...
    try {
      ...
      // Filter out findings without file paths
      final findingsWithFiles = findings.where((f) => f.filePath != null).toList();

      // Count findings per file
      final fileCounts = <String, int>{};
      for (final finding in findingsWithFiles) {
