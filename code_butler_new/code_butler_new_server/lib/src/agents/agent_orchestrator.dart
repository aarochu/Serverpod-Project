...
import 'package:code_butler_new_server/src/agents/reader_agent.dart';
import 'package:code_butler_new_server/src/agents/security_agent.dart';
import 'package:code_butler_new_server/src/agents/performance_agent.dart';
import 'package:code_butler_new_server/src/agents/documentation_agent.dart';
import 'package:code_butler_new_server/src/agents/verifier_agent.dart';
import 'package:code_butler_new_server/src/services/github_service.dart';
import 'package:code_butler_new_server/src/services/file_filter.dart';
import 'package:code_butler_new_server/src/services/repository_cache.dart';
import 'package:code_butler_new_server/src/services/findings_cache.dart';
import 'package:code_butler_new_server/src/generated/protocol.dart';
...
class AgentOrchestrator {
  ...
  Future<void> processReview(int reviewSessionId) async {
    try {
      ...
      // Apply intelligent prioritization (sort by importance)
      sortedFiles.sort((a, b) {
        // Prioritize non-test files, then by path length (shorter = more important)
        final aIsTest = a.path.contains('test') || a.path.contains('spec');
        final bIsTest = b.path.contains('test') || b.path.contains('spec');
  ...
  Future<List<AgentFinding>> _processFileWithTimeout(
    ...
    try {
      return await _analyzeFile(filePath, language, pullRequestId)
          .timeout(Duration(seconds: fileTimeoutSeconds));
    } catch (e) {
      if (e is TimeoutException) {
        session.log('File analysis timed out: $filePath', level: LogLevel.warning);
        return [];
      }
      rethrow;
    }
