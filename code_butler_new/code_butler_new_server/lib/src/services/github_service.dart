...
class GitHubService {
  ...
  void authenticateWithToken(String token) {
    ...
    if (token.isNotEmpty && token != 'YOUR_GITHUB_TOKEN') {
      _github = github.GitHub(auth: github.Authentication.withToken(token));
  ...
  Future<github.Repository> fetchRepository(String owner, String repoName) async {
    if (_github == null) {
    ...
    try {
      final repo = await _github!.repositories.getRepository(
        github.RepositorySlug(owner, repoName),
      );
  ...
  Future<github.PullRequest> fetchPullRequest(
    String owner,
    String repoName,
    int prNumber,
  ) async {
    ...
    try {
      final slug = github.RepositorySlug(owner, repoName);
